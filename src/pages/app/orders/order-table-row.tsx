...
import type { OrderStatusDTO } from '@/@types/order-status-dto'
...
export function OrderTableRow({ order }: OrderTableRowPros) {
  ...
  function updateOrderStatusOnChace(orderId: string, status: OrderStatusDTO) {
  ...
  const { mutateAsync: cancelOrderFn, isPending: iscCancelPending } =
    useMutation({
      mutationFn: cancelOrder,
      async onSuccess(_, { orderId }) {
        updateOrderStatusOnChace(orderId, 'canceled')
      },
    })
  ...
  const { mutateAsync: approveOrderFn, isPending: isApprovePending } =
    useMutation({
      ...
      async onSuccess(_, { orderId }) {
        updateOrderStatusOnChace(orderId, 'processing')
      },
    })
  ...
  const { mutateAsync: dispatchOrderFn, isPending: isDispatchPending } =
    useMutation({
      ...
      async onSuccess(_, { orderId }) {
        updateOrderStatusOnChace(orderId, 'delivering')
      },
    })
  ...
  const { mutateAsync: deliverOrderFn, isPending: isDeliverPending } =
    useMutation({
      ...
      async onSuccess(_, { orderId }) {
        updateOrderStatusOnChace(orderId, 'delivered')
      },
    })
